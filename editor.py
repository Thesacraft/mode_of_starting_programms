    ...
    if(editor_mode == "edit"):
        while True:
            ...
            if(mode in editor.modes):
                ...
                while True:
                    mode_editor = input(f'Do You Wanna create or wanna delete a entry(entrys: {editor.getEntrys(file)})?(exit to exit)(change to change)').strip().lower()
                    if (editor.checkIfchange(mode_editor)):
                        break
                    if(mode_editor == "create" or mode_editor == "delete"):
                        if(mode_editor == "create"):
                            while True:
                                name = input("Prompt a name for the Programm or for a webbrowser \"webbrowser\" (exit to exit)(change to change)!").strip().lower()
                            ...
                            while True:
                                ...
                                entry = input(f"Choose one of ({entrys}) these to delete(exit to exit)(change to change)!").strip().lower()
        ...
        while True:
            mode = input(f"Input the mode that should be deleted(currently availabel modes:{editor.modes})(exit to exit)(change to change)!")
            if(editor.checkIfchange(mode)):
                break
